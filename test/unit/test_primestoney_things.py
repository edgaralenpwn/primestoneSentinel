...
def test_valid_primestone_address():
    from primestonelib import is_valid_primestone_address

    main = valid_primestone_address()
    test = valid_primestone_address('testnet')

    assert is_valid_primestone_address(main) is True
    assert is_valid_primestone_address(main, 'mainnet') is True
    assert is_valid_primestone_address(main, 'testnet') is False

    assert is_valid_primestone_address(test) is False
    assert is_valid_primestone_address(test, 'mainnet') is False
    assert is_valid_primestone_address(test, 'testnet') is True


def test_invalid_primestone_address():
    from primestonelib import is_valid_primestone_address

    main = invalid_primestone_address()
    test = invalid_primestone_address('testnet')

    assert is_valid_primestone_address(main) is False
    assert is_valid_primestone_address(main, 'mainnet') is False
    assert is_valid_primestone_address(main, 'testnet') is False

    assert is_valid_primestone_address(test) is False
    assert is_valid_primestone_address(test, 'mainnet') is False
    assert is_valid_primestone_address(test, 'testnet') is False
...
def test_deterministic_masternode_elections(current_block_hash, mn_list):
    from primestonelib import elect_mn
...
def test_hash_function():
    ...
    hex_hash = "%x" % primestonelib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(primestonelib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(primestonelib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(314.4).quantize(precision)
    assert int(primestonelib.blocks_to_seconds(16616)) == 2612035
