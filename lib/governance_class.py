...
class GovernanceClass(object):
    ...
    def vote(self, primestoned, signal, outcome):
        return self.go.vote(primestoned, signal, outcome)

    # pass thru to GovernanceObject#voted_on
    def voted_on(self, **kwargs):
        return self.go.voted_on(**kwargs)

    def vote_validity(self, primestoned):
        if self.is_valid():
            printdbg("Voting valid! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(primestoned, models.VoteSignals.valid, models.VoteOutcomes.yes)
        else:
            printdbg("Voting INVALID! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(primestoned, models.VoteSignals.valid, models.VoteOutcomes.no)
    ...
    def list(self):
        ...
        # return a dict similar to primestoned "gobject list" output
    ...
    def primestoned_serialise(self):
        import primestonelib
        return primestonelib.SHIM_serialise_for_primestoned(self.serialise())
